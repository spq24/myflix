...
describe UserSignup do
	describe"#sign_up" do
		context "valid personal info and valid card" do
		   ...
		   it "stores the cusotmer token from stripe" do
	       	 UserSignup.new(Fabricate.build(:user)).sign_up("some_stripe_token", nil)
	      ...
	      it "sends out email containing the user's name with valid inputs" do
	        ...
	        expect(ActionMailer::Base.deliveries.last.body).to eq('Joe Smith')
	      end
		end
		...
		context "valid personal info and declined card" do
	     it "does not create a new user record" do
	        ...
	        StripeWrapper::Customer.should_receive(:create).and_return(ccustomer)
	    ...
	    context 'with invalid personal info' do
	      it "does not create the user" do
	      	UserSignup.new(User.new(emai: "kevin@example.com")).sign_up('123412', nil)
	        expect(User.count).to eq(0)
	      end
